...
//        }
    ...
    private static boolean compareWithZh1(List<BeanArray> zhList, List<BeanArray> cur) {
        for (int i = 0; i < zhList.size(); i++) {
            BeanArray allSource = zhList.get(i);
            BeanArray source = cur.get(i);
            if (!source.key.equals(allSource.key) || allSource.items.size() != source.items.size()) {
                return false;
            } else {
                for (int i1 = 0; i1 < source.items.size(); i1++) {
                    if (allSource.items.get(i1).equals("") && !source.items.get(i1).equals("")) {
                        return false;
                    }
                }
            }
